        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Energy Transformer Action Selection: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study energy-based transformer robot policies. The submission-hardened claim is intentionally narrow: Replace next-action decoding with energy-ranked feasible action manifolds. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Energy transformer action selection should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.409 $\pm$ 0.018 & 0.429 $\pm$ 0.015 & 0.326 $\pm$ 0.010 \\
Data-augmented WAM & 0.482 $\pm$ 0.015 & 0.517 $\pm$ 0.009 & 0.426 $\pm$ 0.015 \\
Scalar uncertainty & 0.568 $\pm$ 0.016 & 0.578 $\pm$ 0.016 & 0.502 $\pm$ 0.013 \\
Ensemble disagreement & 0.591 $\pm$ 0.004 & 0.629 $\pm$ 0.015 & 0.550 $\pm$ 0.014 \\
Conformal risk filter & 0.598 $\pm$ 0.017 & 0.629 $\pm$ 0.009 & 0.561 $\pm$ 0.009 \\
Proposed branch mechanism & 0.687 $\pm$ 0.010 & 0.723 $\pm$ 0.015 & 0.653 $\pm$ 0.010 \\
Oracle branch upper bound & 0.765 $\pm$ 0.009 & 0.797 $\pm$ 0.014 & 0.754 $\pm$ 0.009 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.660 $\pm$ 0.010 & all components \\
Minus branch atlas & 0.540 $\pm$ 0.008 & removes explicit unrealized futures \\
Minus tail risk & 0.575 $\pm$ 0.011 & uses mean predicted value only \\
Minus diagnostic probe & 0.607 $\pm$ 0.015 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.628 $\pm$ 0.011 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/62.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Energy Transformer Action Selection is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
